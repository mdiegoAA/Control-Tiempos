function ServiciosViewModel() {
    var self = this;

    self.id = ko.observable(undefined);
    self.codigo = ko.observable(undefined);
    self.nombre = ko.observable(undefined);
    self.esGenerico = ko.observable(undefined);

    self.refrescar = function () {
        $("#servicios-repeater").repeater("render");
    };

    self.mostrar = function () {
        $("#servicio-form").resetValidation();
        $("#servicio-modal").modal({ backdrop: "static" });
    };
    ...
    self.guardar = function () {
        if ($("#servicio-form").valid()) {

            var servicio = {
                id: self.id(),
                codigo: self.codigo(),
                nombre: self.nombre(),
                esGenerico: self.esGenerico()
            };
    ...
    self.editar = function (id) {
        ...
        self.obtenerPorId(id).done(function (data) {
            ...
            self.esGenerico(data.esGenerico);
            self.mostrar();
        }).fail(function (jqXHR) {
            bootbox.alert(jqXHR.responseJSON.exceptionMessage);
        });
